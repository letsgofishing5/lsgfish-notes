...
2. git reflog：可以查看已经被被删除的版本号，可以用户版本向前回滚
...
3. 版本回退的三种方法：
   ...
   2. 使用**^**符号：该命令只能版本后退
      ...
      1. **git reset --hard 索引值^**，**一个^符号**代表后退一版本，**两个代表**后退两个版本
...
```txt
*.tmp //所有以tmp结尾的文件都忽略
/.tmp //当前文件下的以tmp结尾的文件都忽略
css/  //忽略css目录，包括目录下的所有文件
!lib.a //跟踪lib.a文件，即便前面已经忽略了.a文件
```

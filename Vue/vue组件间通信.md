## 组件间多种通信方式:
    1). 组件间通信(父向子): props
        子如何修改父的数据？
            转存一份在子组件中直接修改（各个子组件中数据独立的）
            通过vue自定义事件 通知父组件自己改（各个子组件中数据公用的）
    2). 组件间通信（子向父 非父子）: vue自定义事件与事件总线
    3). 组件间通信: 作用域插槽slot-scope
    4). 组件间通信: vuex


    5). 组件间通信高级2: v-model
    6). 组件间通信高级3: .sync 属性修饰符
    7). 组件间通信高级4: $attrs与$listeners
    8). 组件间通信高级5: $children与$parent
    9). provide&inject

    10). 如果有路由环境考验通过url进行组件数据传递
    11). localStroage seesionStroage
...
## vue自定义事件与事件总线
    ...
    vue自定义事件：
        ...
        $event: 就是分发自定义事件时指定的data数据（$emit的第二个参数）
        $event可以是任意类型, 甚至可以没有
        用来实现子向父组件通信, 功能相当于函数类型的props

    native修饰符：
        将vue的自定义事件 转为 原生DOM事件

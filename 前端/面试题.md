...
4. 组件传值
   ...
   答：父传子：props，子传父：$emit

5. 如何获取dom?

   答：通过ref来获取dom，ref="domName" 用法：this.$refs.domName
...
9. .native修饰的作用
   ...
   1. 获取组件里的原生事件，比如click事件，不写native则表示当前组件按钮事件
...
1. 使用axios发送异步请求时，返回一个对象，对象有name、age两个属性，我现在要取name属性值，怎么取？

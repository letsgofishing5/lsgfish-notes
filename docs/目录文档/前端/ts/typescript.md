...
1. esModuleInterop 是 TypeScript 配置文件（tsconfig.json）中的一个选项，用于改进 CommonJS 模块（CJS）与 ES6 模块（ESM）之间的互操作性。当 esModuleInterop 设置为 true 时，TypeScript 编译器会在编译过程中对某些模块导入方式进行转换，以便更平滑地在同时使用 CJS 和 ESM 模块的项目中工作。
